...
% initial pseudoranges for nine satellites
true.psrL1 = [NaN; NaN; 22991819.2577186; NaN; NaN; NaN; 24517655.6958688; ...
    22049669.1716807; 22566722.2032708; NaN; NaN; NaN; NaN; NaN; ...
    NaN; 20348204.2143794; NaN; NaN; 23457324.8303492; 23733615.9121063; ...
    NaN; NaN; 20634164.0473434; NaN; NaN; NaN; 21699817.3121622; ...
    NaN; NaN; NaN; 24287244.0460344; NaN; round(86465.0000000016)];

% initial transmit time
true.initialReceiveTime = 86465;

% initial satellite positions
[svpos,svvel,svclk,svprns] = get_sv_position3(ephemerides,true.psrL1);

% available satellites
sigs = logical(~isnan(true.psrL1(1:32)) & ~isnan(svpos(1,:))');

% available satellite prn numbers
true.prns = constants.prnList(sigs);

% number of visible satellites
true.numChannels = length(true.prns);

% iterative satellite position calculator
for jjj = 1:7
    for iii = 1:length(true.prns)
        transitTime = norm(svpos(:,true.prns(iii))-true.xyz)/constants.c;
        [svpos(:,true.prns(iii)),svvel(:,true.prns(iii)),svclk(true.prns(iii))] = ...
            calc_sv_pos3(ephemerides2(:,true.prns(iii)),...
            true.initialReceiveTime - transitTime,...
            transitTime);
        true.range(true.prns(iii)) = norm(svpos(:,true.prns(iii))-true.xyz);
    end
end
...
% save initial satellite position, velocity, and clock bias
init.svpos = svpos;
init.svvel = svvel;
init.svclk = svclk;
...
% Set true carrier to noise
cNo = nomCn0*ones(length(true.prns),1);

% Setup true channel data
for kk = 1:length(true.prns)

    % true channel data contains all parameters to which the scalar or
    % vector tracking channel parameters should converge. Includes true
    % clock bias, satellite clock bias, code phase, carrier phase, code
    % frequency, carrier frequency, receive time, transmit time

    % calculate transmit time including receiver and satellite clock biases
    true.channel(true.prns(kk)).receiveTime = true.initialReceiveTime;

    % record receive time
    true.channel(true.prns(kk)).transmitTime = true.channel(true.prns(kk)).receiveTime - ((true.range(true.prns(kk)) + true.ionoDelay + true.tropDelay + true.clock.bias - svclk(true.prns(kk))*constants.c)/constants.c);

    % assign prn number
    true.channel(true.prns(kk)).prn = true.prns(kk);

    % calculate los velocity and carrier frequency
    ...
    % record carrier frequency
    true.channel(true.prns(kk)).carrFreq = velData(2);

    % record doppler
    true.channel(true.prns(kk)).doppler = velData(2) - constants.IF;

    % calculate code frequency from doppler
    true.channel(true.prns(kk)).codeFreq = true.channel(true.prns(kk)).doppler * constants.chipRate/constants.L1 + constants.chipRate;

    % seconds into initial code period
    partialCodePeriodSec = mod(true.channel(true.prns(kk)).transmitTime,1e-3);

    % chips into initial code period
    partialCodePeriodChips = partialCodePeriodSec*constants.chipRate;

    % radians into initial code period
    partialCodePeriodRadians = partialCodePeriodSec*true.channel(true.prns(kk)).carrFreq;

    % initialize code phase
    true.channel(true.prns(kk)).codePhase = partialCodePeriodChips;

    % initialize carrier phase
    true.channel(true.prns(kk)).phase = - (true.range(true.prns(kk)) + true.ionoDelay + true.tropDelay + true.clock.bias - svclk(true.prns(kk))*constants.c)/constants.wavelength;

    % set carrier to noise ratio
    true.channel(true.prns(kk)).cnoL1 = 10^(cNo(kk)/10);

    % record initial code phase
    true.channel(true.prns(kk)).initial_code_phase = true.channel(true.prns(kk)).codePhase;

    % recordn initial transmitTime
    true.channel(true.prns(kk)).initial_transmitTime = true.channel(true.prns(kk)).transmitTime;

end

% state vector for comparison to estimated state vector
true.state.vector = [true.xyz(1);true.vel.xyz(1);...
    true.xyz(2);true.vel.xyz(2);...
    true.xyz(3);true.vel.xyz(3);...
    true.clock.bias;true.clock.drift];
